...
#from . import ico_interface
from . import gvtoken_interface

#implements: ico_interface

state: uint256
isPaused: bool
teamAllocator: address
gvToken: gvtoken_interface
...
#@ always ensures: self.isPaused and self.state != 4 ==> gvtoken_totalSupply(self.gvToken) == old(gvtoken_totalSupply(self.gvToken))
# Property spec_07 (altered)
#@ inter contract invariant: self._init ==> not frozen(self.gvToken) ==> self.state == 4
# Property spec_08
#@ always ensures: migration_agent(self.gvToken) == 0 and old(gvtoken_totalSupply(self.gvToken)) > gvtoken_totalSupply(self.gvToken) ==> self.state == 3 or self.state == 2
...
def __init__(token: address, op: address, ta: address):
    ...
    self.gvToken = gvtoken_interface(token)
