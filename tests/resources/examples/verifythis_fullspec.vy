...
#@ performs: reallocate(self.bet if (self.guess == (convert(keccak256_inv(self.hashedNumber), uint256) % 2 == 0)) else 0, to=self.player)
#@ performs: payout[wei](2 * self.bet if (self.guess == (convert(keccak256_inv(self.hashedNumber), uint256) % 2 == 0)) else 0, actor=self.player)
@public
