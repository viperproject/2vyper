...
#:: ExpectedOutput(invariant.violated:assertion.false) | Label(INV)
#@ invariant: self._init ==> self._value <= mapping(self.token_A)[self]
...
def increase() -> bool:
    ...
    if self.token_A.get() != MAX_UINT256:
        #:: ExpectedOutput(carbon)(call.invariant:assertion.false, INV)
